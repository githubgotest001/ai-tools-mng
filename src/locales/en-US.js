export default {
  ...
  tokenForm: {
    ...
    clearTag: 'Clear tag',
    addTag: 'Add',
    tagLimitHint: 'Up to {max} tags',
    tagLimitReached: 'Limit of {max} reached. Remove one to add another.',
    optional: 'Optional',
    required: 'Required',
    save: 'Save',
    update: 'Update',
  ...
  cursorUsage: {
    ...
    eventsTruncated: 'Showing first {shown} of {total} events; totals cover all of them',
    eventsFetchCapped: 'Large dataset: loaded first {loaded} of {total} events; stats are based on loaded data',
    freeCredit: 'Free',
    freeCreditTooltip: 'Covered by bonus credits, actual charge $0.00',
    partialChargeTooltip: 'Actual charge ${charged}',
    chargedLabel: 'Charged',
    totalCostHint: 'Actual charged amount in the selected range (plan + on-demand). Bonus-credit and non-charged usage are excluded; bonus usage is listed separately below.',
    freeCreditDeducted: 'Bonus credits used',
    freeCreditDeductedHint: 'Usage in this range covered by granted bonus credits (promos/trials), valued at API list price. Not included in total cost; marked "Free" in the details.',
    onPlanHint: 'This billing cycle. Included: usage charged against the plan quota; Bonus: granted bonus credits consumed (shown as free in the official dashboard, not billed).',
    onDemandHint: 'On-demand spend this billing cycle (pay-as-you-go beyond the plan quota).',
  ...
  platform: {
    ...
    cursor: {
      ...
      sessionInvalid: 'Session Expired',
      sessionInvalidHint: 'Cursor rejected the last quota refresh as unauthorized. Re-import a fresh token; the plan and usage shown are from the last successful refresh.',
      sessionInvalidAt: 'Detected at {time}',
      quotaRefreshedAt: 'Quota refreshed at',
      viewSyncQueueTooltip: 'Click to view pending sync queue',
      syncQueueTitle: 'Sync Queue',
      syncQueueUpsertsTitle: 'Pending Updates',
      ...
      sync: 'Sync',
      noQuotaData: 'No quota data available',
      quotaRemainingHint: 'Percentages show remaining (available) quota — higher means more left, not used',
      membershipType: 'Plan',
      autoUsage: 'Auto left',
      apiUsage: 'API left',
      ...
      planAvailable: 'Plan left',
      totalAvailable: 'Total left',
      autoShort: 'Auto',
      apiShort: 'API',
      grokBotShort: 'Bot',
      planAvailableHint: 'Matches the dashboard banner "You\u2019ve used X% of your included usage", computed as includedSpend / limit (limit is the plan price in cents). This is not one of the Spending page bars \u2014 they use different denominators, so the numbers are expected to differ a lot.',
      totalAvailableHint: 'Matches the Total bar on the Spending page: spend across both usage pools divided by their combined budget. The pools are different sizes, so this is not the average of Auto and API.',
      autoAvailableHint: 'Matches the Cursor Models pool (Grok / Composer) on the Spending page, which is what actually gates you. Since 2026-08-24 this pool\u2019s limit went up and Auto bills at the routed model\u2019s rate, so the number can jump.',
      ...
      grokBotAvailableHint: 'Remaining Grok Bot weekly allowance, separate from the monthly Auto / API pools. Cursor only exposes a percentage for it, no dollar amount.',
      poolSpend: 'This cycle {spend}',
      poolLimitEstimated: 'Spend comes from this cycle\u2019s aggregated usage events. Cursor does not publish the pool budgets, so the total is estimated as spend / percent used.',
