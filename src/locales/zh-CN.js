export default {
  ...
  tokenForm: {
    ...
    clearTag: '清空标签',
    addTag: '添加',
    tagLimitHint: '最多可添加 {max} 个标签',
    tagLimitReached: '已达上限（{max} 个），移除一个才能继续添加',
    optional: '可选',
    required: '必填项',
    save: '保存',
    update: '更新',
  ...
  cursorUsage: {
    ...
    rangeThisMonth: '本月',
    rangeLast7Days: '近7天',
    rangeLast30Days: '近30天',
    rangeAll: '全部',
    dateTo: '至',
    allModels: '全部模型',
    eventsCount: '共 {count} 条',
    totalWithCount: '合计（{count} 条）',
    modelsCount: '{count} 个模型',
    eventsTruncated: '记录较多，仅显示前 {shown} 条（共 {total} 条），合计包含全部记录',
    eventsFetchCapped: '数据量过大，仅加载前 {loaded} / {total} 条，统计基于已加载数据',
    freeCredit: '赠送',
    freeCreditTooltip: '由赠送额度（Free Credit）抵扣，实际计费 $0.00',
    partialChargeTooltip: '实际计费 ${charged}',
    chargedLabel: '计费',
    totalCostHint: '所选时间范围内实际计费的金额（套餐内计费 + 按需计费）。赠送额度抵扣和未计费请求不计入，赠送部分在下方单独列出。',
    freeCreditDeducted: '赠送抵扣',
    freeCreditDeductedHint: '当前范围内由赠送额度（活动/试用等发放）抵扣的用量，按 API 标价统计，不计入总费用；对应明细中标为「赠送」的记录。',
    onPlanHint: '本账期套餐用量。Included：套餐内额度已使用金额；Bonus：已消耗的赠送额度（官方后台显示为免费，不计入实际计费）。',
    onDemandHint: '本账期按需计费金额（超出套餐后按量付费的部分）。',
  ...
  platform: {
    ...
    cursor: {
      ...
      sessionInvalidHint: '刷新配额时 Cursor 返回未授权，需要重新获取 Token 覆盖导入；套餐与用量仍是上次成功刷新的数据',
      sessionInvalidAt: '检测时间 {time}',
      quotaRefreshedAt: '配额刷新时间',
      viewSyncQueueTooltip: '点击查看待同步队列',
      syncQueueTitle: '同步队列',
      syncQueueUpsertsTitle: '待更新',
      ...
      sync: '同步',
      noQuotaData: '暂无配额数据',
      quotaRemainingHint: '百分比均为剩余可用比例，越高剩得越多，不是已用比例',
      membershipType: '订阅计划',
      autoUsage: 'Auto 可用',
      apiUsage: 'API 可用',
      ...
      apiShort: 'API',
      grokBotShort: 'Bot',
      planAvailableHint: '对应仪表盘横幅“You’ve used X% of your included usage”，按 includedSpend / limit 计算（limit 是套餐月费美分）。它不是 Spending 页那几条进度条，两者分母不同，数值本来就会差很多',
      totalAvailableHint: '对应 Spending 页 Total 进度条：两个用量池的花费除以两池合计预算。因为两池预算不等，它不是 Auto 与 API 的平均值',
      autoAvailableHint: '对应 Spending 页 Cursor Models 池（Grok / Composer），也是实际限流依据。2026-08-24 起该池上限上调、Auto 改为按路由到的模型计费，比例会明显跳变',
      apiAvailableHint: '对应 Spending 页 Other Models 池（第三方模型）。该池未随 2026-08-24 的调整扩容，先耗尽的通常是它',
      grokBotAvailableHint: 'Grok Bot 本周剩余额度，与 Auto / API 月度池分开计算（官方只给百分比，没有金额）',
      poolSpend: '本账期 {spend}',
      poolLimitEstimated: '已用金额来自本账期聚合用量事件；池总额官方不提供，按「已用金额 ÷ 已用百分比」反推，为估算值',
